# NV Flow documentation

Use this page as the documentation index.

## Guides

- [Nodes and controls](NODES.md) describes every NV Flow node and its main inputs.
- [Short and long video processing](VIDEO_PROCESSING.md) explains which path to
  use, FPS behavior, memory limits, chunk resume, encoding, audio, and progress.
- [Upscaling and source-quality tuning](UPSCALING.md) explains quality modes,
  automatic source analysis, and external ComfyUI upscale models.
- [Example workflows](WORKFLOWS.md) lists every included graph and the setup each
  one requires.

For installation and the high-level path comparison, return to the
[project landing page](../README.md).
